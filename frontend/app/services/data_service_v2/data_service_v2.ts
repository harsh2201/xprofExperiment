...
export class DataServiceV2 implements DataServiceV2Interface {
    ...
    return this.httpClient.get<T>(url, options).pipe(
      catchError((error: HttpErrorResponse) => {
        ...
        if (error.status === 0) {
          ...
          errorMessage =
            'There was an error in the requested URL ' +
            urlObj.pathname +
            urlObj.search +
            '.<br><br>' +
            '<b>message:</b> ' +
  ...
  private getHTTPParamsForDataQuery(
    ...
    {updateSearchParams = true}: {updateSearchParams?: boolean} = {},
  ): HttpParams {
  ...
  getData(
    ...
    }: {ignoreError?: boolean; updateSearchParams?: boolean} = {},
  ...
  getSmartSuggestions(
    ...
    let params = this.getHTTPParamsForDataQuery(
      sessionId,
      tool,
      '',
      parameters,
    ...
    if (
      new URLSearchParams(window.location.search).get('refresh_suggestion') ===
      'true'
    ) {
      params = params.set('refresh_suggestion', 'true');
    }
    return this.get<SmartSuggestionReport>(this.pathPrefix + DATA_API, {
      'params': params,
    });
  }
  ...
  getGraphVizUri(sessionId: string, params: Map<string, string>): string {
    ...
    return `${window.origin}/${this.pathPrefix}/${DATA_API}?${searchParams.toString()}${GRAPHVIZ_PAN_ZOOM_CONTROL}`;
  }
  ...
  getHosts(run: string, tool: string): Observable<HostMetadata[]> {
    ...
    return this.httpClient.get(this.pathPrefix + HOSTS_API, {
      params,
    }) as Observable<HostMetadata[]>;
  }
  ...
  getOpProfileData(
    ...
    return this.getData(sessionId, 'op_profile', host, params, {
      ignoreError: false,
      updateSearchParams: false,
    }) as Observable<DataTable>;
  }
    ...
    return [
      {
        name: 'Hbm',
        value:
          data?.bandwidthUtilizationPercents?.[
  ...
  getCustomCallRegvizLink(
    ...
    if (moduleName && opName) {
      ...
      return `${window.parent.location.origin}/${
        DATA_API
      }?tag=graph_viewer&module_name=${moduleName}&node_name=${
        opName
      }&run=${sessionId}&type=custom_call&regviz=true#profile`;
    }
    return '';
  }
  ...
  getRunTools(run: string): Observable<string[]> {
    ...
    return this.get(this.pathPrefix + RUN_TOOLS_API, {
      'params': params,
    }) as Observable<string[]>;
  }
  ...
  captureProfile(
    ...
    return this.httpClient.get<CaptureProfileResponse>(
      this.pathPrefix + CAPTURE_PROFILE_API,
      {params},
    );
  }
