...
void Timeline::HandleMouseRelease() {
  if (ImGui::IsMouseReleased(0)) {
    ...
    HandleBookmarkAddition(is_click);
...
void Timeline::HandleBookmarkAddition(bool is_click) {
  if (bookmarks_enabled_ && is_click &&
      (ImGui::GetIO().KeyCtrl || ImGui::GetIO().KeySuper)) {
    const ImRect timeline_area = GetTimelineArea();
    if (ImGui::IsMouseHoveringRect(timeline_area.Min, timeline_area.Max)) {
      Microseconds time = PixelToTime(
...
void Timeline::HandleSelectionOrTimeRangeAddition() {
...
void Timeline::DrawBookmarks(Pixel timeline_width,
  ...
  for (size_t i = 0; i < bookmarks_.size(); ++i) {
    const Microseconds time = bookmarks_[i];
    ...
    std::string label = FormatTime(time);
    ImGui::PushFont(fonts::label_small);
    ImVec2 label_size = ImGui::CalcTextSize(label.c_str());
    ImVec2 label_pos = ImVec2(x + kBookmarkLabelPadding,
                              timeline_area.Min.y + kBookmarkLabelPadding);
    ...
    ImRect hover_rect(label_pos, button_rect.Max);
...
void Timeline::RemoveBookmark(Microseconds time) {
  if (!bookmarks_enabled_) return;
  auto it = std::find(bookmarks_.begin(), bookmarks_.end(), time);
  if (it != bookmarks_.end()) {
    bookmarks_.erase(it);
    if (redraw_callback_) redraw_callback_();
  }
