...
from tensorflow.compiler.xla import (
    xla_data_pb2,
)
...
def format_shape(shape_proto: Any) -> str:
  ...
  try:
    element_type_str = xla_data_pb2.PrimitiveType.Name(
        shape_proto.element_type
    ).lower()
...
def _initialize_primitive_type_mapping() -> dict[int, int]:
  ...
  for spec_name, bit_width in _PRIMITIVE_TYPE_SPECS:
    val = None
    try:
      val = xla_data_pb2.PrimitiveType.Value(spec_name)
    except (ValueError, AttributeError):
      try:
        val = getattr(xla_data_pb2, spec_name, None)
...
def get_tpu_lane_size(
  ...
  if element_type in LANE_SIZE_BY_PRIMITIVE_TYPE:
    ...
    try:
      name = xla_data_pb2.PrimitiveType.Name(element_type)
